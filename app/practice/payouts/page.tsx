    ...
    <div className="flex flex-col" style={{ background: '#0d0d16', height: '100dvh', fontFamily: 'Georgia, serif' }}>
      ...
      <div className="flex items-center justify-between px-3 py-1.5"
        ...
        <button
          ...
          style={{
            ...
            color: 'rgba(232,200,106,0.55)', fontSize: 18, lineHeight: 1,
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div style={{
          ...
          >
            ALL<br />
            <span style={{ fontSize: 8, opacity: 0.7 }}>Random</span>
          </button>

          <div style={{ height: 1, background: 'rgba(255,255,255,0.07)', margin: '2px 6px' }} />
          ...
          {ZONES.map(z => {
            ...
            return (
              ...
              >
                <div style={{ fontSize: 11, fontWeight: 900, letterSpacing: '0.03em' }}>{m.short}</div>
        ...
        {/* ── Right: drill area ── */}
        <div className="felt flex-1 flex flex-col items-center justify-between py-4 px-3 overflow-hidden min-h-0">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, flex: 1, justifyContent: 'center', width: '100%' }}>
            ...
            {betAmount === 0 && phase === 'pick-chip' ? (
              <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                Tap chips to place a bet
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div style={{ color: '#e8c86a', fontSize: 38, fontWeight: 900, fontFamily: 'Georgia, serif', lineHeight: 1 }}>
                  ${betAmount.toLocaleString()}
                </div>
                {phase === 'answering' && (
                  <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                    Side bet profit?
                  </div>
                )}
                {phase === 'answered' && answer && (
                  <div style={{ color: answer.correct ? '#4ade80' : '#f87171', fontSize: 12, fontWeight: 900, textAlign: 'center' }}>
                    {answer.correct
                ...
                {options.map(opt => {
                  ...
                  return (
                    <button key={opt} onClick={() => handleAnswer(opt)} disabled={phase === 'answered'} style={{
                      height: 54, fontSize: 17, fontWeight: 900,
                      borderRadius: 11, cursor: phase === 'answering' ? 'pointer' : 'default',
                      touchAction: 'manipulation',
                      background: bg, border: `2px solid ${border}`, color,
                      transition: 'all 0.12s',
                      boxShadow: isCorrect && phase === 'answered' ? '0 0 12px rgba(74,222,128,0.35)' : 'none',
                    }}>
                      +${opt.toLocaleString()}
                    </button>
                  );
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {phase === 'pick-chip' && (
              <>
                ...
                <div style={{
                  ...
                  {CHIP_DEFS.map(chip => (
                    <Chip3D key={chip.value} value={chip.value} size={46} onClick={() => addChip(chip.value)} />
                  ))}
                </div>
                <button
                  onClick={randomBet}
                  style={{
                    width: '100%', height: 38, fontSize: 12, fontWeight: 800,
                ...
                {betAmount > 0 && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button
                      ...
                      style={{
                        flex: 1, height: 42, fontSize: 12, fontWeight: 700,
                    ...
                    <button
                      onClick={confirmBet}
                      style={{
                        flex: 2, height: 42, fontSize: 13, fontWeight: 900,
                        borderRadius: 10, background: meta.color,
            ...
            {phase === 'answered' && (
              <button onClick={nextRound} style={{
                width: '100%', height: 50, fontSize: 15, fontWeight: 900,
